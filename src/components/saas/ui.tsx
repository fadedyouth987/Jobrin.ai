...
export function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <div className={`jobrin-card rounded-2xl border border-slate-200 bg-white shadow-sm ${className}`}>{children}</div>;
}

export function PrimaryButton({ children, className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`jobrin-primary rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}>{children}</button>;
}

export function SecondaryButton({ children, className = '', ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`jobrin-secondary rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}>{children}</button>;
}
...
export function PageIntro({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return <div className={`mb-5 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 text-sm leading-6 text-slate-600 ${className}`}><span className="mt-0.5 flex h-5 w-5 flex-none items-center justify-center rounded-full bg-indigo-100 text-xs font-black text-indigo-700">i</span><div className="min-w-0">{children}</div></div>;
}
...
export function StatCard({ icon, label, value, sub, className = '' }: { icon?: React.ReactNode; label: string; value: React.ReactNode; sub?: string; className?: string }) {
  return <Card className={`p-4 ${className}`}><div className="flex items-center justify-between gap-2"><p className="text-sm font-semibold text-slate-600">{label}</p>{icon && <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">{icon}</span>}</div><div className="mt-1.5 text-3xl font-bold tracking-tight text-slate-950">{value}</div>{sub && <p className="mt-1 text-xs leading-4 text-slate-400">{sub}</p>}</Card>;
}
      ...
      <div className="divide-y divide-slate-100">
        {pending.map((item, index) => (
          <AppLink key={item.href + item.label} href={item.href} className="flex items-start gap-3 p-4 transition hover:bg-indigo-50/40">
            <span className="mt-0.5 flex h-6 w-6 flex-none items-center justify-center rounded-full bg-slate-100 text-xs font-black text-slate-500">{doneCount + index + 1}</span>
    ...
    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 px-6 py-12 text-center">
      ...
      {steps && steps.length > 0 && (
        <ol className="mx-auto mt-4 max-w-md space-y-1.5 text-left">
          {steps.map((step, index) => <li key={index} className="flex items-start gap-2 text-xs leading-5 text-slate-500"><span className="mt-0.5 flex h-4 w-4 flex-none items-center justify-center rounded-full bg-slate-200 text-xs font-black text-slate-600">{index + 1}</span>{step}</li>)}
        </ol>
      )}
