...
const PrivacyPage = lazy(() =>
  import("./pages/PublicLegalPages").then((module) => ({
...
function Routes() {
  ...
  const quoteMatch = path.match(/^\/quote\/([A-Za-z0-9_-]+)$/);
  if (quoteMatch) return <PublicQuotePage token={quoteMatch[1]} />;
  const bookMatch = path.match(/^\/book\/([a-z0-9-]+)$/i);
  if (bookMatch) return <PublicBookingPage slug={bookMatch[1]} />;
...
function AuthEntry({ auth }: { auth: ReturnType<typeof useAuth> }) {
  useEffect(() => {
    if (!auth.loading && auth.session)
      navigate(auth.workspaceId ? "/app" : "/onboarding", true);
...
function PaymentOutcome({ complete }: { complete: boolean }) {
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-5">
      <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        <div
          className={`mx-auto flex h-14 w-14 items-center justify-center rounded-full text-2xl ${complete ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}
        >
          {complete ? "✓" : "—"}
        </div>
        ...
        <h1 className="mt-2 text-3xl font-black">
          {complete ? "Payment submitted" : "Payment cancelled"}
        </h1>
        <p className="mt-3 text-sm leading-6 text-slate-600">
          {complete
            ? "Stripe is confirming the payment securely. The business records it only after receiving a signed confirmation from Stripe."
            : "No payment was recorded by Jobrin.ai. You can return to the original payment link if you still need to pay."}
        </p>
        <p className="mt-6 text-xs text-slate-400">
          Jobrin.ai does not receive or store your card number or security code.
        </p>
      </div>
    </div>
  );
