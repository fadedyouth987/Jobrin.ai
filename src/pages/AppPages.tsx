...
export function ReceptionistPage(){
  const {workspaceId}=useAuth();const query=useData<any>('/api/receptionist',{profile:null,readiness:{}});const calls=useData<any>('/api/receptionist/calls',{calls:[]});const [form,setForm]=useState<any>(receptionistDefaults);const [message,setMessage]=useState('');const [error,setError]=useState('');const [saving,setSaving]=useState(false);const [simHistory,setSimHistory]=useState<Array<{role:'user'|'assistant';content:string}>>([]);const [simInput,setSimInput]=useState('');const [simBusy,setSimBusy]=useState(false);const [simError,setSimError]=useState('');const [simMode,setSimMode]=useState('receptionist');const [signNote,setSignNote]=useState('');const [signBusy,setSignBusy]=useState(false);
...
export function AutomationsPage() {
  ...
  const [form,setForm]=useState({name:'',description:'',trigger_key:'job.completed',tool:'business.report'});
  const create=async(e:React.FormEvent)=>{e.preventDefault();if(!workspaceId)return;setBusy('create');setError('');try{const input={question:form.description||'Summarise business performance'};await apiFetch('/api/intelligence/automations',{method:'POST',body:JSON.stringify({name:form.name,description:form.description,trigger_key:form.trigger_key,definition:{conditions:[],steps:[{tool:'business.report',input}]},schedule_cron:null,timezone:'Australia/Adelaide'})},workspaceId);setOpen(false);setForm({name:'',description:'',trigger_key:'job.completed',tool:'business.report'});await query.refresh()}catch(err:any){setError(err.message)}finally{setBusy('')}};
