...
import {
  Bell, CalendarDays, ChevronDown, Home, Inbox, KeyRound, LogOut, Menu, Moon, Phone, Search, Sun, X
} from 'lucide-react';
...
export default function AppShell() {
  ...
  return <div className="jobrin-workspace min-h-screen bg-slate-50 text-slate-950">
    ...
    <nav className="jobrin-bottom-nav fixed inset-x-0 bottom-0 z-30 flex border-t border-slate-200 bg-white lg:hidden">
      {([['/app','Today',Home],['/app/operator/phone','AI Phone',Phone],['/app/inbox','Inbox',Inbox],['/app/schedule','Schedule',CalendarDays]] as NavItem[]).map(([href,label,Icon])=>{const active=path===href||(href!=='/app'&&path.startsWith(`${href}/`));const featured=href==='/app/operator/phone';return <AppLink key={href} href={href} className={`jobrin-bottom-link flex flex-1 flex-col items-center gap-0.5 py-2.5 text-[10px] font-semibold ${active?'is-active text-indigo-600':'text-slate-400'} ${featured?'is-featured':''}`}><Icon className="h-4 w-4"/>{label}</AppLink>})}
