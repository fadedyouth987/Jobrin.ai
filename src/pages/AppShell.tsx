...
function CommandPalette({ items, onClose }: { items: NavItem[]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-black/55 p-4 pt-[8vh] backdrop-blur-sm" onClick={onClose}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Find a page" className="jobrin-palette mx-auto max-w-xl overflow-hidden rounded-xl border border-[color:var(--jobrin-chrome-border)] shadow-2xl" onClick={(e)=>e.stopPropagation()}>
      <input value={query} onChange={(e)=>setQuery(e.target.value)} aria-label="Search pages and features" placeholder="Search pages and actions…"
        ...
        className="w-full border-0 border-b border-[color:var(--jobrin-chrome-border)] bg-transparent px-5 py-4 text-sm text-[color:var(--jobrin-chrome-ink)] outline-none placeholder:text-[color:var(--jobrin-chrome-muted)]"/>
      <div className="max-h-80 overflow-y-auto p-2">
        {filtered.length?filtered.map((item)=><button key={item.href} onClick={()=>{navigate(item.href);onClose()}} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm text-[color:var(--jobrin-chrome-ink)] hover:bg-[color:var(--jobrin-elevated)]">
          <item.Icon className="h-4 w-4 text-[color:var(--jobrin-chrome-muted)]"/><span className="flex-1 font-medium">{item.label}</span>{item.soon&&<span className="rounded-full border border-[color:var(--jobrin-chrome-border)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[color:var(--jobrin-chrome-muted)]">coming soon</span>}
        </button>):<p className="p-4 text-sm text-[color:var(--jobrin-chrome-muted)]">No matches.</p>}
      </div>
      <div className="flex items-center justify-between border-t border-[color:var(--jobrin-chrome-border)] px-4 py-2 text-[11px] text-[color:var(--jobrin-chrome-muted)]"><span>↑↓ browse · ↵ open · esc close</span><span>Ctrl K</span></div>
    </div>
  </div>;
}
  ...
  return <div className="jobrin-workspace min-h-screen bg-[color:var(--jobrin-canvas)] text-[color:var(--jobrin-ink)]">
    ...
    <header className="jobrin-deck fixed inset-x-0 top-0 z-50 flex h-14 items-center gap-2 px-3 sm:px-4">
      ...
      <div className="relative mx-auto w-full max-w-md">
        <button onClick={()=>setPaletteOpen(true)} title="Search (Ctrl+K)" className="flex w-full items-center gap-2 rounded-lg border border-[color:var(--jobrin-chrome-border)] bg-[color:var(--jobrin-elevated)] px-3 py-2 text-xs text-[color:var(--jobrin-chrome-muted)] hover:border-[color:var(--jobrin-chrome-border)] hover:text-[color:var(--jobrin-chrome-ink)]">
          <Search className="h-3.5 w-3.5"/><span className="flex-1 text-left">Search pages & actions…</span><kbd className="hidden rounded border border-[color:var(--jobrin-chrome-border)] px-1.5 text-[10px] sm:inline">Ctrl K</kbd>
        </button>
      </div>
      <div className="flex items-center gap-1">
        <div className="jobrin-deck-workspace relative hidden md:block"><select value={auth.workspaceId} onChange={e=>auth.setWorkspaceId(e.target.value)} aria-label="Switch workspace" className="w-44 appearance-none rounded-lg border border-[color:var(--jobrin-chrome-border)] bg-[color:var(--jobrin-elevated)] px-3 py-2 pr-8 text-xs font-semibold text-[color:var(--jobrin-chrome-ink)] outline-none"><option value={auth.workspaceId}>{auth.workspace?.name}</option>{auth.workspaces.filter(w=>w.id!==auth.workspaceId).map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-2 top-2.5 h-3.5 w-3.5 text-[color:var(--jobrin-chrome-muted)]"/></div>
    ...
    {mobile&&<aside aria-label="Workspace navigation" role="dialog" aria-modal={true} className="jobrin-sidebar fixed inset-y-0 left-0 z-50 w-[280px] translate-x-0">
      ...
      <div className="border-b border-[color:var(--jobrin-chrome-border)] p-3"><label className="block text-[10px] font-bold uppercase tracking-wider text-[color:var(--jobrin-chrome-muted)]">Workspace</label><div className="relative mt-1"><select value={auth.workspaceId} onChange={e=>auth.setWorkspaceId(e.target.value)} className="w-full appearance-none rounded-lg border border-[color:var(--jobrin-chrome-border)] bg-[color:var(--jobrin-elevated)] px-3 py-2.5 pr-8 text-sm font-semibold text-[color:var(--jobrin-chrome-ink)] outline-none"><option value={auth.workspaceId}>{auth.workspace?.name}</option>{auth.workspaces.filter(w=>w.id!==auth.workspaceId).map(w=><option key={w.id} value={w.id}>{w.name}</option>)}</select><ChevronDown className="pointer-events-none absolute right-2.5 top-3 h-4 w-4 text-[color:var(--jobrin-chrome-muted)]"/></div><div className="mt-2 flex items-center justify-between px-1 text-[11px]"><span className="capitalize text-[color:var(--jobrin-chrome-muted)]">{auth.workspace?.role}</span><span className="rounded-full border border-[color:var(--jobrin-rail-active)]/30 bg-[color:var(--jobrin-rail-active)]/10 px-2 py-0.5 font-semibold capitalize text-[color:var(--jobrin-rail-active)]">{auth.workspace?.plan}</span></div></div>
      <nav className="jobrin-sidebar-nav overflow-y-auto px-2 py-3">{visibleGroups.map((group,gi)=>group.label? <details key={gi} className="mb-3" open={group.items.some(([href])=>path===href||(href!=='/app'&&path.startsWith(`${href}/`)))}><summary className="cursor-pointer list-none rounded-lg px-3 py-2 text-sm font-semibold text-[color:var(--jobrin-chrome-muted)] hover:bg-[color:var(--jobrin-elevated)]">{group.label}</summary><div className="mt-1 border-l border-[color:var(--jobrin-chrome-border)] pl-1">{group.items.map(([href,label,Icon,soon])=>{const active=isActive(href);return <AppLink key={href} href={href} onClick={()=>setMobile(false)} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition ${active?'bg-[color:var(--jobrin-rail-active)]/12 text-[color:var(--jobrin-chrome-ink)]':'text-[color:var(--jobrin-chrome-muted)] hover:bg-[color:var(--jobrin-elevated)] hover:text-[color:var(--jobrin-chrome-ink)]'}`}><Icon className={`h-4 w-4 flex-none ${active?'text-[color:var(--jobrin-rail-active)]':'text-[color:var(--jobrin-chrome-muted)]'}`}/><span className="min-w-0 flex-1">{label}</span>{soon===true&&<span className="flex-none rounded-full border border-[color:var(--jobrin-chrome-border)] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[color:var(--jobrin-chrome-muted)]">soon</span>}</AppLink>})}</div></details>:<div key={gi} className="mb-3">{group.items.map(([href,label,Icon,soon])=>{const active=isActive(href);return <AppLink key={href} href={href} onClick={()=>setMobile(false)} className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition ${active?'bg-[color:var(--jobrin-rail-active)]/12 text-[color:var(--jobrin-chrome-ink)]':'text-[color:var(--jobrin-chrome-muted)] hover:bg-[color:var(--jobrin-elevated)] hover:text-[color:var(--jobrin-chrome-ink)]'}`}><Icon className={`h-4 w-4 ${active?'text-[color:var(--jobrin-rail-active)]':'text-[color:var(--jobrin-chrome-muted)]'}`}/><span className="flex-1">{label}</span>{soon===true&&<span className="rounded-full border border-[color:var(--jobrin-chrome-border)] px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[color:var(--jobrin-chrome-muted)]">soon</span>}</AppLink>})}</div>)}</nav>
      <div className="jobrin-sidebar-account border-t border-[color:var(--jobrin-chrome-border)] p-3"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#f2c24e] text-sm font-black text-[#14201b]">{(auth.user?.user_metadata?.display_name||auth.user?.email||'U')[0]?.toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-[color:var(--jobrin-chrome-ink)]">{auth.user?.user_metadata?.display_name||auth.user?.email}</p><p className="truncate text-[10px] text-[color:var(--jobrin-chrome-muted)]">{auth.user?.email}</p></div><button aria-label="Log out" title="Log out" onClick={async()=>{await logoutUser();navigate('/login',true)}} className="rounded-lg p-2 text-[color:var(--jobrin-chrome-muted)] hover:bg-[color:var(--jobrin-elevated)] hover:text-[color:var(--jobrin-chrome-ink)]"><LogOut className="h-4 w-4"/></button></div></div>
    </aside>}
