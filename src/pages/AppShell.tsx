...
import {
  ...
  MarketingPage, ModulePage, NotificationsPage, OperationsListPage, OperatorPage, ReviewsPage, SecuritySettingsPage, SettingsPage, TeamPage,
} from './AppPages';
import { ReceptionistWorkspace, type ReceptionistSection } from './receptionist/ReceptionistWorkspace';
...
function CommandPalette({ items, onClose }: { items: NavItem[]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[60] overflow-y-auto bg-slate-950/50 p-4 pt-[8vh]" onClick={onClose}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Find a page" className="mx-auto max-w-xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl" onClick={(e)=>e.stopPropagation()}>
...
export default function AppShell() {
  ...
  return <div className="jobrin-workspace min-h-screen bg-slate-50 text-slate-950">
    ...
    <aside aria-label="Workspace navigation" role={mobile?'dialog':undefined} aria-modal={mobile?true:undefined} className={`jobrin-sidebar fixed inset-y-0 left-0 z-50 w-[270px] border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${mobile?'translate-x-0':'-translate-x-full'}`}>
      <div className="flex h-16 items-center justify-between border-b border-slate-100 px-4"><AppLink href="/app" onClick={()=>setMobile(false)} className="flex items-center gap-2.5 font-black tracking-tight"><span className="brand-mark flex h-9 w-9 items-center justify-center rounded-xl bg-indigo-600 text-white">J</span>Jobrin.ai</AppLink><button onClick={()=>setMobile(false)} aria-label="Close navigation" className="rounded-lg p-2 text-slate-500 lg:hidden"><X className="h-4 w-4"/></button></div>
      ...
      <nav className="jobrin-sidebar-nav overflow-y-auto px-2 py-3">{visibleGroups.map((group,gi)=>group.label? <details key={gi} className="mb-3" open={group.items.some(([href])=>path===href||(href!=='/app'&&path.startsWith(`${href}/`)))}><summary className="cursor-pointer list-none rounded-xl px-3 py-2 text-sm font-semibold tracking-normal text-slate-600 hover:bg-slate-50">{group.label}</summary><div className="mt-1 border-l border-slate-200 pl-1">{group.items.map(([href,label,Icon,soon])=>{const active=path===href||(href!=='/app'&&path.startsWith(`${href}/`));return <AppLink key={href} href={href} onClick={()=>setMobile(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition ${active?'bg-indigo-50 text-indigo-700':'text-slate-600 hover:bg-slate-100 hover:text-slate-950'}`}><Icon className={`h-4 w-4 flex-none ${active?'text-indigo-600':'text-slate-400'}`}/><span className="min-w-0 flex-1">{label}</span>{soon===true&&<span className="flex-none rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-400">soon</span>}</AppLink>})}</div></details>:<div key={gi} className="mb-3">{group.items.map(([href,label,Icon,soon])=>{const active=path===href||(href!=='/app'&&path.startsWith(`${href}/`));return <AppLink key={href} href={href} onClick={()=>setMobile(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition ${active?'bg-indigo-50 text-indigo-700':'text-slate-600 hover:bg-slate-100 hover:text-slate-950'}`}><Icon className={`h-4 w-4 ${active?'text-indigo-600':'text-slate-400'}`}/><span className="flex-1">{label}</span>{soon===true&&<span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-400">soon</span>}</AppLink>})}</div>)}</nav>
...
function routePage(path:string) {
  const receptionistMatch=path.match(/^\/app\/operator\/phone(?:\/(configure|handling|knowledge|test|calls|insights))?$/);if(receptionistMatch)return <ReceptionistWorkspace section={(receptionistMatch[1]||'overview') as ReceptionistSection}/>;
