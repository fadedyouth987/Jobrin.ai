...
const raw = {
  ...
  DEPLOYMENT_STAGE:
    process.env.DEPLOYMENT_STAGE ?? process.env.NODE_ENV ?? "development",
  ...
  REQUIRE_AAL2_SENSITIVE:
    process.env.REQUIRE_AAL2_SENSITIVE ??
...
export function assertProductionSecrets() {
  ...
  const missing = required
    .filter(([, value]) => !value || /REPLACE|YOUR_|missing/i.test(value))
    .map(([name]) => name);
  if (missing.length)
  ...
  if (!/^(rk|sk)_(test|live)_/.test(env.STRIPE_SECRET_KEY))
    throw new Error(
      "STRIPE_SECRET_KEY must be a restricted or secret Stripe key",
    );
  ...
  if (!/^sk-(?:proj-)?[A-Za-z0-9_-]+$/.test(env.OPENAI_API_KEY))
    throw new Error("OPENAI_API_KEY has an invalid format");
  if (!/^AC[a-fA-F0-9]{32}$/.test(env.TWILIO_ACCOUNT_SID))
    throw new Error("TWILIO_ACCOUNT_SID has an invalid format");
  if (!/^\+[1-9]\d{7,14}$/.test(env.TWILIO_PHONE_NUMBER))
    throw new Error("TWILIO_PHONE_NUMBER must use E.164 format");
  if (!env.EMAIL_API_KEY.startsWith("re_"))
    throw new Error("EMAIL_API_KEY has an invalid Resend format");
  if (
    !/^[^<>\s@]+@[^<>\s@]+\.[^<>\s@]+|^.+<[^<>\s@]+@[^<>\s@]+\.[^<>\s@]+>$/.test(
      env.EMAIL_FROM,
    )
  )
    throw new Error("EMAIL_FROM has an invalid format");
