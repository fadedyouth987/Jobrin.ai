...
import {
  stripe,
  stripeConfigured as stripeProviderConfigured,
  stripeWebhookConfigured,
} from "../providers/stripe";
import { PLAN_ENTITLEMENTS, type PlanKey } from "../../shared/plans";

// Re-exported for the invoice checkout route in operations.ts, which shares
// the same singleton and the same configured gate.
export { stripe };

const PLAN_FEATURES = PLAN_ENTITLEMENTS;
...
async function applySubscription(subscription: Stripe.Subscription) {
  ...
  const plan =
    ...
    (subscription.metadata?.plan as PlanKey | undefined);
  const workspaceId = subscription.metadata?.workspace_id;
  if (!workspaceId || !plan || !PLAN_FEATURES[plan])
  ...
  const { error } = await supabaseAdmin.rpc("apply_subscription_state", {
    ...
    target_period_end: sub.current_period_end
...
stripeWebhookRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (claimError)
      return res.status(500).json({ error: "STRIPE_EVENT_CLAIM_FAILED" });
    if (claim !== "claimed")
      return res.json({
        received: true,
        duplicate: claim === "duplicate",
        inProgress: claim === "in_progress",
      });
    ...
    try {
      ...
      if (
        ...
        if (settlementError)
          throw new Error(
            `INVOICE_PAYMENT_SETTLEMENT_FAILED:${settlementError.message}`,
          );
      }
      ...
      return res
        .status(500)
        .json({ error: "STRIPE_WEBHOOK_PROCESSING_FAILED" });
    }
  }),
);
...
router.get(
  ...
  asyncRoute(async (req: AuthenticatedRequest, res) => {
    ...
    const accessState = db
      .rpc("get_workspace_access_state", { target_workspace: req.workspaceId! })
      .maybeSingle();
    const fullSubscription = ["owner", "admin"].includes(
      req.workspaceRole || "",
    )
      ? db
          .from("subscriptions")
          .select(
            "plan,status,current_period_end,cancel_at_period_end,trial_ends_at,grace_period_ends_at,stripe_customer_id,stripe_subscription_id,updated_at",
          )
    ...
    const [
      ...
      supabaseAdmin
        .from("usage_counters")
        .select("metric_key,quantity,period_start")
        .eq("workspace_id", req.workspaceId!)
    ...
    if (error || activeUsersError)
      return res.status(500).json({ error: "SUBSCRIPTION_READ_FAILED" });
    const pricesConfigured = [
      env.STRIPE_PRICE_STARTER,
      env.STRIPE_PRICE_GROWTH,
      env.STRIPE_PRICE_OPERATOR,
    ].every((price) => price.startsWith("price_"));
    res.json({
      subscription: ownerSubscription ?? access,
      entitlements: entitlements ?? [],
      usage: [
        { metric_key: "usage.users", quantity: activeUsers ?? 0, period_start: periodStart },
        ...(usageCounters ?? []).filter((item) => item.metric_key !== "usage.users"),
      ],
      stripeConfigured: stripeProviderConfigured() && pricesConfigured,
    });
  }),
);
...
router.post(
  ...
  asyncRoute(async (req: AuthenticatedRequest, res) => {
    ...
    if (!price || !price.startsWith("price_"))
      return res
        .status(503)
        .json({ error: "STRIPE_PRICE_NOT_CONFIGURED", plan });
    ...
    if (
      ...
      return res
        .status(409)
        .json({ error: "ACTIVE_SUBSCRIPTION_EXISTS", usePortal: true });
    }
    ...
    if (!customerId) {
      ...
      if (existingSub) {
        ...
        if (customerLinkError)
          return res.status(500).json({ error: "STRIPE_CUSTOMER_LINK_FAILED" });
      } else {
        const { error: customerLinkError } = await supabaseAdmin
          .from("subscriptions")
          .insert({
    ...
    const providedKey = String(
      req.header("idempotency-key") || req.requestId || "",
    ).slice(0, 200);
...
router.get(
  ...
  asyncRoute(async (req: AuthenticatedRequest, res) => {
    ...
    const ownerWorkspace =
      session.metadata?.workspace_id || session.client_reference_id;
    if (ownerWorkspace !== req.workspaceId)
