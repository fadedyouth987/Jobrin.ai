...
router.get('/jobs/:id', asyncRoute(async (req: AuthenticatedRequest, res) => {
  ...
  const [appointmentResult, timeEntriesResult, materialsResult, quotesResult, invoicesResult] = await Promise.all([
    job.appointment_id
      ? db.from('appointments').select('id,title,status,starts_at,ends_at,address_text,assigned_user_id').eq('workspace_id', workspaceId).eq('id', job.appointment_id).maybeSingle()
  ...
  const relatedError = [appointmentResult.error, timeEntriesResult.error, materialsResult.error, quotesResult.error, invoicesResult.error].find(Boolean);
...
router.patch('/jobs/:id/status', requireRole('owner','admin','manager','staff'), validateBody(z.object({ status: z.enum(['new','scheduled','on_the_way','in_progress','completed','invoiced','paid','cancelled']) })), asyncRoute(async (req: AuthenticatedRequest, res) => {
  ...
  if (req.body.status === 'completed') {
    // Best-effort: fires any active "job completed" automations with real
    // jobId/customerId/serviceIds inputs. Never allowed to break this request.
    void queueAutomationRun(req.workspaceId!, 'job.completed', { jobId: data.id, customerId: data.customer_id ?? null, serviceIds: data.service_id ? [data.service_id] : [] }).catch(() => {});
  }
  res.json({ job: data });
}));
