...
const MIME_TYPE_EXTENSIONS = { 'image/jpeg': 'jpg', 'image/png': 'png', 'image/webp': 'webp', 'image/gif': 'gif', 'video/mp4': 'mp4' } as const;
...
router.post('/service-agreements/:id/generate', requireRole('owner', 'admin', 'manager'), asyncRoute(async (req: AuthenticatedRequest, res) => {
  ...
  const originalNextServiceAt = agreement.next_service_at;

  // Compare-and-swap the cadence advance BEFORE inserting the job. Two
  // concurrent calls both read the same next_service_at, but only the CAS
  // that still matches the original value wins the .eq('next_service_at', ...)
  // guard — the loser affects zero rows and is treated as "someone else
  // already generated this cycle" instead of creating a duplicate job.
  const casQuery = supabaseAdmin.from('service_agreements')
    ...
    .select('id,name,status,cadence,price_cents,next_service_at');
  const { data: updated, error: updateError } = originalNextServiceAt
    ? await casQuery.eq('next_service_at', originalNextServiceAt).maybeSingle()
    : await casQuery.is('next_service_at', null).maybeSingle();
  if (updateError) return res.status(500).json({ error: 'AGREEMENT_SCHEDULE_UPDATE_FAILED', message: updateError.message });
  if (!updated) return res.status(409).json({ error: 'AGREEMENT_ALREADY_GENERATED', message: 'This cycle was already generated by another request.' });
...
router.post('/jobs/:id/photos', requireRole('owner', 'admin', 'manager', 'staff'), validateBody(z.object({
  ...
  // The storage key extension is derived from the validated mime_type enum,
  // never from the client-supplied file_name — a crafted file_name like
  // "x.xyz/../../../evil" would otherwise splice a path-traversal segment
  // straight into the shared asset bucket key.
  const ext = MIME_TYPE_EXTENSIONS[req.body.mime_type];
...
router.patch('/quotes/:id/send', requireRole('owner','admin','manager','staff'), asyncRoute(async (req: AuthenticatedRequest, res) => {
  ...
  // Everything that can fail runs BEFORE the quote is marked sent, matching
  // the invoice-send route below. A failed email leaves the quote in draft —
  // retryable — instead of stranded as "sent" with a link nobody received.
  // No email on file is a deliberate manual-delivery path (hand the link to
  // the customer yourself), not a delivery failure, so it still marks sent.
...
router.post('/quotes/:id/convert', requireRole('owner','admin','manager','staff'), asyncRoute(async (req: AuthenticatedRequest, res) => {
  ...
  // No separate "does an invoice already exist" check: that was a
  // check-then-act race where two concurrent converts could both pass the
  // check and both insert. Instead we attempt the insert directly and rely
  // on invoices_quote_id_active_idx (a partial unique index on
  // (quote_id) where quote_id is not null and status != 'void') to reject
  // the loser with a 23505 unique_violation, which we translate back into
  // the same QUOTE_ALREADY_INVOICED response the old check produced.
  ...
  if (invoiceError) {
    if ((invoiceError as { code?: string }).code === '23505') {
      const { data: existing } = await db.from('invoices').select('id').eq('workspace_id', req.workspaceId!).eq('quote_id', quote.id).neq('status', 'void').limit(1).maybeSingle();
      return res.status(409).json({ error: 'QUOTE_ALREADY_INVOICED', invoiceId: existing?.id });
    }
    return res.status(400).json({ error: 'INVOICE_CREATE_FAILED', message: invoiceError.message });
  }
